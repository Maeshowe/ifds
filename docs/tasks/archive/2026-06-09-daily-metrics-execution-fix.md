Status: DONE
Updated: 2026-06-09
Note: **DEPLOYOLVA + Day 17 esti EOD-on ÉLES VERIFIKÁCIÓ ZÖLD** (commit 8c28a4b + 937dbcc, deploy a53a5a9, 1938 passing). A 6/9 22:10 cron `daily_metrics/2026-06-09.json`: **#1** ACHC slippage filled=25.50 (IBKR fill, +0.71%, NEM a state planned 25.32); **#2** trades.details=2 (**VNO TP1** +230.47 13:30Z + **WST TIME_STOP_MOC** +178.41 20:00Z, exit_type a fill-timestampből); `matched=2 warnings=0` broker_realized_pnl, 0 hiba. 6/8 backfill alkalmazva (TKR +1.43%, NSA -0.05%). **Egy másodlagos finding** (NEM blokkoló): az eod_report 22:11 Telegram `Trades: 1`-et mutatott (eod_report.py:665 `len(trades)` a clientId-12 saját fill-jeiből, ami nem látja a cross-client WST MOC-ot) — a persisted daily_metrics helyes (2). **Follow-up task: `docs/tasks/2026-06-10-eod-telegram-persisted-details.md`** (az eod a már megírt daily_metrics-ből olvassa a Trades-számot/details-t). ——— Eredeti finding: a `slippage_per_ticker::*::filled` a state planned-et tükrözte (TKR 6/8: 131.83 vs valós 133.71), és a `trades.details` kihagyta a 21:40 MOC fill-eket.
