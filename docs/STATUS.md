...
<!-- Utolsó frissítés: 2026-06-24 (Day ~25, szerda) — **UW greek-exposure FLIP VÉGREHAJTVA + UW 07-04 deadline felmérve** (push ..dbbeaa6, **1969 passing**, 0 failure). **A flip kész**: kétlépcsős config-réteg runbook (STEP 1 `7c3fd55` deploy_intraday.sh conditional `--config` + Mini-local `prod_overrides.json`; STEP 2 06-24 `{"tuning":{"uw_gex_fetch_enabled":false}}`) → **GEX Polygon-only** a Mini-n; minden kapu zöld volt (source=uw==0, live on/off diff PASS). **UW = nulla döntési hatás.** **Nyitott (egyetlen)**: flip post-verify a mai 14:30 run után (0× greek-exposure, source=polygon, regime+exclusion változatlan) → §11.6 flip-lezárás. **UW 07-04**: a közös sub (MID+IFDS) 07-04-én meghal; **IFDS bizonyítottan biztonságos** (validator UW=optional, phase0 non-critical, blankolt-kulcs dry-run "Pipeline CAN proceed") — a tiszta lépés: `IFDS_UW_API_KEY` törlése a Mini `.env`-ből (Tamás, kódmunka nélkül). De-scope dok 07-04 checklist + handoff (`docs/handoff/2026-06-24-*`). ⚠️ **Drawdown**: cumulative 1716 → **555** (06-22 −$398, 06-23 −$331) — a review tárgya. **Production-kód FAGYOTT Day 63-ig** (a flip volt az utolsó prod-változás). **Következő CC**: `signal_attribution` data-loader wiring (freeze-safe, spec §6.1 pinned). **0 nyitott task.** -->
