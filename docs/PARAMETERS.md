...
- `M_VIX`: deaktivált (`m_vix_enabled=False`, Task #2)
...
### Swing Sizing (2026-05-18, Day 63 §3.7, §3.11 — Döntés 7, 11)

| Kulcs | Érték | Phase | Hatás |
|-------|-------|-------|-------|
| `swing_sizing_enabled` | True | P6 | Aktív → új sizing képlet + sector-balanced greedy fill. False → legacy multiplier chain + `_apply_position_limits` cascade |
| `swing_risk_per_trade_pct` | 0.0035 | P6 | 0.35% risk per entry ($350/$100k) — Decision 7 |
...
A `_select_swing_entries` sector-balanced greedy fill (Decision 10):
1. Rangsor `combined_score` (= swing EWMA) szerint desc
2. Iteratíven hozzáad, ha a per-sector running notional + új entry ≤ `swing_sector_cap_pct × equity`
