...
| Dátum | Változás |
|---|---|
| 2026-07-25 | **Második fordulat LEZÁRVA — HYP-005 verdikt megerősítve (Tamás 2026-07-24).** h5/h7 **PARK_UNTIL_SWING_POWER**, h1/h3 **KILL**; registry `Status: PARKED`. **Ébresztési család pre-reg: {h5,h7}, Šidák m=2**, a retest-adat ELŐTT rögzítve (garden-of-forking-paths zárva). Ledger A-0005..A-0008 `human_confirmed`, az override (auto=KILL→PARK) auditálható. **Két bug/rés kifogva a bevitel közben**: (1) a `confirm_decision` nem volt idempotens → dupla-confirm korrumpálta az `auto_decision`-t (`3e64a5d`, +2 teszt, immutábilis auto_decision); helyreállítás a commitolt tiszta ledgerből (a commit a source-of-truth). (2) floor=6 érzékenység dokumentálva (verdikt-stabil [5,7]). Két /learn szabály (`51a08c8`): értékelő-motor csak pre-reg-hez igazítható; ledger confirm idempotencia. **2209 passing.** A ritmus innentől: pénteki batch (cost-model + PARK auto-retest), verdikt-javaslat CC-től, megerősítés Tamástól; a b-trió ~szept. közepén érik. |
