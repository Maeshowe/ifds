...
## 10-Q / 10-K SEC Filing Exclusion — Phase 2 bővítés (W19+ scope) — ÚJ 2026-05-04

**Forrás:** AGNC 2026-05-04 eset (-$398.42 LOSS_EXIT). Az AGNC Q1 2026 earnings release **ápr 21-én** volt (13 nappal korábban, kívül a 7 napos earnings_exclusion_days ablakon), de a részletes **10-Q SEC filing** **május 4-én** (hétfő) jelent meg. A piac a 10-Q reszelet -$148M comprehensive loss-ra reagált: az ár 1 órán belül -2.02%-ra zuhant az IFDS submit után, pontosan a LOSS_EXIT küszöbön.

**A jelenség:**

A Phase 2 `_exclude_earnings()` jelenleg **csak earnings release dátumokat** szűri (FMP `/stable/earnings` + `earnings-calendar` endpointokból). A **10-Q (quarterly) és 10-K (annual)** SEC filing-ek **NEM szerepelnek** ezekben az endpointokban, pedig érdemi piaci hatást termelhetnek — különösen REIT-eknél (AGNC, NLY, TWO, MFA stb.), ahol az adjusted EPS és a GAAP comprehensive income jelentősen eltérhet.

**Megoldási irány:**

```python
# Phase 2 bővítés
def _exclude_sec_filings(tickers, exclusion_days, logger):
    """Exclude tickers with upcoming 10-Q or 10-K SEC filings."""
    today = date.today()
    to_date = today + timedelta(days=exclusion_days)
    
    # SEC EDGAR API: ingyenes, schedule + recent filings
    # https://data.sec.gov/submissions/CIK{cik}.json
    
    for ticker in tickers:
        cik = sec_lookup_cik(ticker.symbol)
        if not cik:
            continue
        
        next_filing = sec_get_next_filing(cik, types=["10-Q", "10-K"])
        if next_filing and today <= next_filing.date <= to_date:
            yield ticker  # exclude
```

**Adátforrás:**

- **SEC EDGAR API** (ingyenes, korlátozott rate limit ~10 req/sec)
- `https://data.sec.gov/submissions/CIK{cik}.json` — cég recent filings
- `https://www.sec.gov/cgi-bin/browse-edgar` — query interface
- Alternatíva: FMP `/stable/sec-filings` endpoint (ha létezik — ellenőrizendő)

**Cseh kihívások:**

- **CIK lookup:** ticker → SEC CIK szám mapping (egyszer-cache-eltő, FMP profile endpointból elérhető)
- **Filing schedule** ismert datással: a 10-Q általában earnings release-t követő 1-2 hétben, ha a cég következetes
- **10-Q "unscheduled":** néha váratlanul jelennek meg — a binary event risk emiatt nem 100%-ban elkerülhető
...
- M_contradiction (CC `f6c4d9e`, 2026-05-02 deployed): a contradiction signal **NEM** fired-olt sem a DTE-re (máj 1), sem az AGNC-re (máj 4) — struktúrális kiegészítésül szolgál
- Phase 2 `earnings_exclusion_days` (jelenleg 7): a 10-Q exclusion **mellé** épülne, NEM helyettítési
- BC26+ scope-ban a binary event risk teljesebb lefedése (Fed meeting dátumok, FOMC minutes, CPI release, stb.) — az összes "scheduled high-impact event" bővíthető

**Státusz:** RÖGZÍTVE 2026-05-04. CC-re vár W19+ alatt.
