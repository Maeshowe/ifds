...
| 11.6 | 2026-06-18 | **`uw_gex_fetch_enabled` flag** (default **True**, dormant) — a UW greek-exposure primary kikapcsolhatóvá tétele → Polygon-only GEX. **Output-invariáns** flip-OFF: `source=uw==0` mind a **92 mért napon** (`verify_gex_uw_invariance.py`), + unit-proof: UW=None → `FallbackGEXProvider==Polygon-only`. Egyben freeze-*javító*: megszünteti a latens UW-recovery confoundot (Day 20-63 nincs forrás-flicker) | output-invariáns cleanup (bizonyított üres diff) | **NEM** — a flip-OFF a Polygon-only-t teszi az egyetlen forrássá, ami a realizált viselkedés (UW 100% bukik) | `<flag commit>` (default True dormant); a Mini-flip-OFF a gated deploy a live on/off diff UTÁN |
| 11.8 | 2026-06-24 | **console settings-footer risk-sor display-fix** — a `_print_config_table` a legacy `runtime.risk_per_trade_pct` (0.7%, a halott day-trade path, phase6:941) helyett a **`tuning.swing_risk_per_trade_pct`** (0.35%, a realizált swing path, phase6:1371) értéket írja ki. A footer eddig félrevezetően 0.7%/$700-at mutatott, miközben a tényleges sizing 0.35%/$350 (a 06-24 live run: RBC $328 / NSA $348 / TDG $315 igazolja). A súly- és sector-cap sorok HELYESEK voltak (0.60/0.10/0.30, max 2). | display-only (§4.2/1 carve-out) | **NEM** — kizárólag a kiírt footer, a sizing logika érintetlen (a swing path már 0.35%-ot használt) | `<this commit>` |
| 11.9 | 2026-07-11 | **eod_report trades CSV writer P1 fix** — a `trades_{date}.csv` mostantól a hiteles ledgerekből épül (`build_trade_report_from_ledger`), nem a clientId-12 fill-rekonstrukcióból. A régi builder 3 defektet okozott (2026-07-08 review §6.2, broker-verifikálva): (1) **self-reentry mis-pair** (PFGC új entry-ár $115.25 × régi exit → −$180.81 a valós +$370.17 helyett), (2) **hiányos exit-lista** (3/7, csak az eod-fills ablak), (3) **metaadat-szennyezés** (mind „MOC", plan-score, N/A sector). Source-pairing: exit_type/entry_score/sector → `pending_exits`; entry-basis/exit/pnl → `daily_metrics::trades::details`; sl/tp → `swing_positions` (best-effort). **07-08 regenerálva** (`.bak.pre_csvfix`). Audit (`scripts/analysis/trades_csv_consistency_check.py`): 35 nap, 32 OK, **3 divergens** (06-09/06-10/06-11, pre-06-09-fix éra) — NEM regenerálva (verifikáció kell, Dev-chat-döntés). | display/tracking-fix (§4.2/1 carve-out) | **NEM** — a CSV csak display + analízis-scripteket feed-el; a `cumulative_pnl.json` (record_pending_exits SOLE writer), a `daily_metrics::details` (broker) és a review-1a (daily_metricsből) érintetlen. +4 teszt, **1985 passing** | `<this commit>` |
| 11.7 | 2026-06-24 | **UW-GEX flip VÉGREHAJTVA + post-verifikált** — Mini-local `prod_overrides.json = {"tuning":{"uw_gex_fetch_enabled":false}}` (mtime 06-24 08:54), a `deploy_intraday.sh` `--config` mechanizmuson (STEP 1) keresztül aktív. GEX **produkcióban Polygon-only**. Post-verify a 06-24 14:30 live runon: `greek-exposure` hívás-szám **0** (0×429), `source=uw` **0**, Phase 5 normál (Analyzed 40 / Passed 37 / Excluded NEGATIVE 3), M_gex=1.000, 0 error/exception | output-invariáns cleanup (bizonyított üres diff, §11.6) | **NEM** — Polygon-only a realizált viselkedés (UW 100% bukik) | `7c3fd55` (STEP 1 mechanizmus) + Mini-local `prod_overrides.json` (gitignored) |
| 11.10 | 2026-07-17 | **07-15 Mini-outage manuális exit reconcile + könyvelés** — a 07-15 áramszünet (07-15 ~12:10 → 07-16 este) alatt ITT/XPO **max_hold/TIME_STOP exit elakadt** (a 07-14 22:00 eval flag-elte, a 07-15 21:40 végrehajtás kimaradt). Tamás **manuálisan zárta** 07-15 15:52-53 CEST (ITT 26 @ 192.15, XPO 21 @ 205.60; broker realized **+$211.61 / +$51.04**). 07-17-én (Mini vissza): (a) ITT/XPO eltávolítva `state/swing_positions.json`-ból (különben a 07-17 21:40 TIME_STOP-cron flat pozíción **MOC-shortot** nyitott volna); (b) a +$262.65 bekönyvelve a proper mechanizmuson (`pending_exits` 07-15 → `apply_pending_exits` **broker_realized_pnl** ág), **cumulative_pnl.json 228.69 → $491.34**. A Gateway `reqExecutions` a 2 napos fillt nem érte el → a broker realized az IBKR `get_account_trades`-ből (MCP), penny-pontos. **entry_price≠IBKR-basis P1**: a 07-07 manuális belépők tervezett árat írtak state-be (ITT 190.07 vs valós **183.93**; XPO 206.78 vs **203.07**) — a könyvelést NEM érinti (broker-realized), a state-basis kozmetikailag hibás marad. | outage-recovery reconcile + P&L-tracking (bug→intended: a stratégia szándéka szerinti exit realizálása) | **NEM új viselkedés** — a max_hold exit maga a stratégia döntése volt (07-14), csak a végrehajtás csúszott outage miatt. **Day 63 edge-minta: a 07-15/16 outage-napok ÉS az ITT/XPO késett exit KIZÁRVA** (a ~2h csúszás alatt esett ITT −1.32%/XPO −2.01%; realizált +$262.65 vs a 13:10 pre-market unrealized +$428.64 = outage-artifact) | Mini state (gitignored) + `docs/handoff/2026-07-15-power-outage-handoff.md` |
...
| 11.11 | 2026-07-23 | **v2 research cross-section sink deploy** (`08d072d`, FRL spec §4.4, D_A Tamás 2026-07-21) — új write-only sink a Phase 4 után: `state/research_cross_section/YYYY-MM-DD.json.gz`, a TELJES pontozott keresztmetszet nyers al-komponensekkel (a phase4_snapshot csak a ~3-7 winnert menti). Az FRL v2 kutatási sávjának forward-gyűjtése; minden csúszó nap végleges adatveszteség a Day 63 utáni dev-ablakból. Szemantika: **`swing_score` és `legacy_composite` KÜLÖN mezőben** (a scan-matrix egyoszlopos `Total_Score`-ja egy teljes FRL-0 auditba került); tech_filter/danger_zone sorok **null** score-ral, nem 0.0 (dp_pct-hibaosztály guard a forrásnál); a veto külön `sector_vetoed` mezőben (§12.1 Reason-felülírás ide nem ér el). Try/except-guarded: sink-hiba nem állíthatja meg a pipeline-t. **Sink-audit fegyelem**: `@patch` mindkét e2e stackben + `assert called`, PLUSZ új **grep-audit teszt** (`test_runner_sinks_are_all_covered_by_the_patch_stack`), ami a JÖVŐBELI patch-eletlen sinket is elkapja — a §8.1.6-8.1.9 rés (a `write_shadow_snapshot` pont ezen csúszott át a `save_phase4_snapshot` fix után) strukturálisan zárva. **Előfeltétel-1 bizonyítva a deploy ELŐTT** (`faa56d3`): sentinel-fájllal (24h-ra visszadátumozott mtime), a teljes suite után mtime+sha256+fájlhalmaz változatlan, **negatív kontrollal** (patch nélkül a suite tényleg prodba ír, és a grep-audit bukik). Mini-verifikáció a pull után: **2158 passed, 1 skipped** (a skip a statsmodels-referencia, dev-only — a batch a saját NW-implementációt futtatja), `state/research_cross_section/` a suite után **nem jött létre**. | display/tracking carve-out (§4.2/1) — kutatási adatgyűjtés | **NEM** — a sink KIZÁRÓLAG ír; scoring/sizing/exit/cron útvonalat nem érint, egyetlen production döntés sem olvassa. Az egyetlen viselkedés-hatás egy try/except-elt fájlírás a Phase 4 után. | `08d072d` (sink), `faa56d3` (deploy-riport), Mini pull → `d10c8de` |
