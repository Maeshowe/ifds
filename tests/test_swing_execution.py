...
def test_compute_sell_qty_full_exit_actions():
    ...
    for action in (ACTION_HARD_SL, ACTION_MENTAL_SL, ACTION_TP2,
                   ACTION_TRAIL_SL, ACTION_TIME_STOP):
...
def test_build_swing_position_from_sizing():
    ...
    pos = build_swing_position_from_sizing(
        ticker="NVDA", entry_price=200.0, atr=5.0, qty=10,
        entry_date="2026-05-18", config=CFG, sector="XLK", m_target=0.6,
    )
...
def test_eod_eval_returns_hold_in_normal_range():
    ...
    action, _ = evaluate_position_eod(
        pos, today_close=101.0, today_high=102.0, today_low=99.0,
        today_date=date.fromisoformat(ENTRY_DATE), config=CFG, equity=EQUITY,
    )
...
def test_eod_eval_returns_mental_sl_below_2atr():
    ...
    pos = _pos()
    action, _ = evaluate_position_eod(
        pos, today_close=95.0, today_high=98.0, today_low=94.0,
        today_date=date.fromisoformat(ENTRY_DATE), config=CFG, equity=EQUITY,
    )
...
def test_eod_eval_returns_tp1_when_high_reaches_1_5atr():
    ...
    pos = _pos()
    action, _ = evaluate_position_eod(
        pos, today_close=102.5, today_high=103.5, today_low=99.0,
        today_date=date.fromisoformat(ENTRY_DATE), config=CFG, equity=EQUITY,
    )
...
def test_eod_eval_returns_tp2_when_high_reaches_3_0atr():
    ...
    pos = _pos()
    action, _ = evaluate_position_eod(
        pos, today_close=105.0, today_high=106.5, today_low=99.0,
        today_date=date.fromisoformat(ENTRY_DATE), config=CFG, equity=EQUITY,
    )
...
def test_eod_eval_returns_hard_sl_when_weekly_below_minus_8pct():
    ...
    pos = _pos(qty=1000, qty_remaining=1000, stop_level=70.0)  # very-loose stop so MENTAL doesn't fire
    action, updates = evaluate_position_eod(
        pos, today_close=91.0, today_high=91.0, today_low=90.0,
        today_date=date.fromisoformat(ENTRY_DATE), config=CFG, equity=EQUITY,
    )
...
def test_eod_eval_returns_time_stop_day5():
    ...
    pos = _pos()
    action, updates = evaluate_position_eod(
        pos, today_close=101.5, today_high=102.5, today_low=99.0,
        today_date=date.fromisoformat("2026-05-23"),     # 5 days after 2026-05-18
        config=CFG, equity=EQUITY,
    )
...
def test_eod_eval_priority_hard_sl_over_mental_sl():
    ...
    action, _ = evaluate_position_eod(
        pos, today_close=90.0, today_high=92.0, today_low=89.0,
        today_date=date.fromisoformat(ENTRY_DATE), config=CFG, equity=EQUITY,
    )
    assert action == ACTION_HARD_SL
...
def test_eod_eval_priority_tp2_over_tp1():
    ...
    pos = _pos()
    action, _ = evaluate_position_eod(
        pos, today_close=104.0, today_high=106.5, today_low=102.0,
        today_date=date.fromisoformat(ENTRY_DATE), config=CFG, equity=EQUITY,
    )
    assert action == ACTION_TP2
...
def test_trail_sl_initializes_after_tp1_hit():
    ...
    action, updates = evaluate_position_eod(
        pos, today_close=104.0, today_high=104.5, today_low=102.0,
        today_date=date.fromisoformat("2026-05-19"), config=CFG, equity=EQUITY,
    )
    assert action == ACTION_HOLD
...
def test_trail_sl_ratchets_upward_only():
    ...
    action, updates = evaluate_position_eod(
        pos, today_close=103.0, today_high=104.0, today_low=101.5,
        today_date=date.fromisoformat("2026-05-20"), config=CFG, equity=EQUITY,
    )
    assert action == ACTION_HOLD
...
def test_trail_sl_triggers_exit_when_close_below():
    ...
    action, _ = evaluate_position_eod(
        pos, today_close=101.0, today_high=102.5, today_low=100.5,
        today_date=date.fromisoformat("2026-05-21"), config=CFG, equity=EQUITY,
    )
...
def test_evaluate_all_positions_missing_ohlc_stays_hold():
    ...
    pos = _pos()
    updated, exits = evaluate_all_positions(
        [pos], {}, today_date=date.fromisoformat(ENTRY_DATE),
        config=CFG, equity=EQUITY,
    )
...
def test_three_day_swing_lifecycle_integration(tmp_path):
    ...
    # --- Day 1: Submit ---
    pos = build_swing_position_from_sizing(
        ticker="AAPL", entry_price=100.0, atr=2.0, qty=50,
        entry_date="2026-05-18", config=CFG, sector="XLK",
    )
    save_swing_positions(state_file, [pos])
    loaded = load_swing_positions(state_file)
    ...
    updated, exits = evaluate_all_positions(
        loaded, ohlc_day2, today_date=date.fromisoformat("2026-05-19"),
        config=CFG, equity=EQUITY,
    )
    ...
    updated, exits = evaluate_all_positions(
        loaded, ohlc_day2b, today_date=date.fromisoformat("2026-05-19"),
        config=CFG, equity=EQUITY,
    )
    assert exits == []
    ...
    updated, exits = evaluate_all_positions(
        loaded, ohlc_day3, today_date=date.fromisoformat("2026-05-20"),
        config=CFG, equity=EQUITY,
    )
