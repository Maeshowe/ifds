...
def _make_historical_entries(
    n=40, gex_base=100.0, gex_noise=10.0, dex_base=5000.0, dex_noise=500.0
):
...
class TestComputeFactorVolatility:
    ...
    def test_constant_values_zero_var(self):
        ...
        entries = [
            {"gex": 100.0, "dex": 5000.0, "dark_share": 0.3, "block_count": 5.0, "iv_rank": 0.25}
            for _ in range(25)
        ]
        result = _compute_factor_volatility(entries, window=20)
...
class TestUnusualnessWithFactorVol:
    ...
    def test_vol_weighting_increases_score(self):
        ...
        u_vol = _compute_unusualness(
            z_scores, [], weights, [], factor_vol=factor_vol, median_sigmas=median_sigs
        )
    ...
    def test_no_factor_vol_unchanged(self):
        ...
        u2 = _compute_unusualness(z_scores, [], weights, [], factor_vol=None, median_sigmas=None)
