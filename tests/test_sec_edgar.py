...
def test_predict_next_10q_no_history_raises():
    ...
    filings = [{"form": "4", "filingDate": "2026-05-01", "reportDate": "",
                "accessionNumber": "x"}]
...
def test_predict_next_10q_picks_most_recent():
    ...
    filings = [
        {"form": "10-Q", "filingDate": "2025-08-01", "reportDate": "2025-06-28", "accessionNumber": "a"},
        {"form": "10-Q", "filingDate": "2026-01-30", "reportDate": "2025-12-27", "accessionNumber": "b"},
        {"form": "10-Q", "filingDate": "2025-05-02", "reportDate": "2025-03-29", "accessionNumber": "c"},
    ]
...
def _client_with_mock_filings(cache_dir: Path, ua: str, filings_by_ticker: dict[str, list[dict]],
                              ticker_to_cik: dict[str, str] | None = None) -> SecEdgarClient:
...
def test_has_upcoming_within_lookahead(temp_cache, fixed_user_agent):
    ...
    filings = [{"form": "10-Q", "filingDate": last_10q_date.isoformat(),
                "reportDate": "", "accessionNumber": "x"}]
...
def test_agnc_2026_05_04_excluded_retroactively(temp_cache, fixed_user_agent):
    ...
    filings = [{"form": "10-Q", "filingDate": "2026-02-03",
                "reportDate": "2025-12-31", "accessionNumber": "agnc-q1"}]
...
def test_cik_map_resolves_ticker(temp_cache, fixed_user_agent, company_tickers):
    ...
    client._cik_map = {row["ticker"].upper(): f"{row['cik_str']:010d}"
...
def test_cik_map_uses_cache_when_fresh(temp_cache, fixed_user_agent, company_tickers):
    ...
    payload = {"_updated": datetime.now(timezone.utc).isoformat(),
               "map": {row["ticker"].upper(): f"{row['cik_str']:010d}"
                       for row in company_tickers.values()}}
...
def test_cik_map_refresh_on_stale(temp_cache, fixed_user_agent):
    ...
    client = SecEdgarClient(cache_dir=temp_cache, user_agent=fixed_user_agent)
    with patch.object(client, "_fetch_cik_map_remote",
                      return_value={"AAPL": "0000320193"}) as mock_fetch:
        client._load_or_fetch_cik_map()
...
def test_filings_cache_refresh_on_stale(temp_cache, fixed_user_agent, aapl_submissions):
    ...
    cache_file.write_text(json.dumps({"_updated": stale_ts,
...
def test_filings_fail_open_when_api_down_uses_2d_cache(temp_cache, fixed_user_agent, aapl_submissions):
...
def test_rate_limit_sleep_between_calls(temp_cache, fixed_user_agent, aapl_submissions, monkeypatch):
