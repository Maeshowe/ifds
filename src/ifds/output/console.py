...
def print_phase_header(step: int, title: str, goal: str, logic: str, source: str) -> None:
...
def print_diagnostics(diag: DiagnosticsResult) -> None:
    ...
    for h in diag.api_health:
        ...
        print(
            f"  {color}{sym:>4}{Style.RESET_ALL}  {h.provider:<18} {h.endpoint:<35} {ms:>8}{crit}"
        )
    ...
    if diag.macro:
        ...
        vix_color = (
            Fore.GREEN
            if m.vix_regime.value == "low"
            else (Fore.YELLOW if m.vix_regime.value == "normal" else Fore.RED)
        )
        print(
        ...
        if hasattr(m, "cross_asset_regime") and m.cross_asset_regime != "NORMAL":
            ...
            print(
                f"  {ca_color}Cross-Asset: {m.cross_asset_regime}{Style.RESET_ALL}"
...
def print_phase1(result: Phase1Result, prev_bmi: dict[str, Any] | None = None) -> None:
    ...
    print(
        f"  BMI = {regime_color}{bmi.bmi_value:.1f}%{Style.RESET_ALL}"
...
def print_phase2(result: Phase2Result) -> None:
    ...
    print(
        f"  Screened: {result.total_screened}"
...
def _sector_change_arrow(etf: str, prev: dict[str, float] | None, current_mom: float) -> str:
...
def print_sector_table(
    result: Phase3Result,
    prev_sectors: dict[str, float] | None = None,
    benchmark: SectorScore | None = None,
) -> None:
    ...
    print(hdr)
    row_width = (
        6 + 2 + 22 + 2 + 6 + 2 + 12 + 2 + 10 + 2 + 6 + 2 + 6 + 2 + 10 + 2 + 7 + 2 + 14 + 2 + 4
    )  # 115
    if prev_sectors:
        row_width += 5
    print("  " + "-" * row_width)

    sectors_sorted = sorted(result.sector_scores, key=lambda s: s.momentum_5d, reverse=True)
...
def _print_sector_row(
    s: SectorScore, prev_sectors: dict[str, float] | None = None, is_benchmark: bool = False
) -> None:
    ...
    regime_color = (
        Fore.GREEN if regime == "OVERSOLD" else (Fore.RED if regime == "OVERBOUGHT" else Fore.WHITE)
    )
...
def print_scan_summary(result: Phase4Result) -> None:
    ...
    print(
        f"  Breakdown — Tech filter: {result.tech_filter_count}"
...
def print_gex_summary(result: Phase5Result) -> None:
    ...
    # MMS regime distribution (BC15)
    if result.mms_analyses:
        regime_counts: dict[str, int] = {}
        for o in result.mms_analyses:
            regime_counts[o.mm_regime.value] = regime_counts.get(o.mm_regime.value, 0) + 1
        parts = [f"{k}={v}" for k, v in sorted(regime_counts.items())]
        label = "MMS" if result.mms_enabled else "MMS (collect-only)"
...
def print_final_summary(result: Phase6Result, ctx: PipelineContext) -> None:
    ...
    print(
        f"  Excluded — sector limit: {result.excluded_sector_limit}"
    ...
    if result.freshness_applied_count:
        print(
            Fore.CYAN + f"  Freshness Alpha applied to {result.freshness_applied_count} ticker(s)"
        )
    ...
    if positions:
        ...
        for i, pos in enumerate(positions[:20], 1):
            ...
            score_color = (
                Fore.GREEN
                if pos.combined_score >= 80
                else (Fore.YELLOW if pos.combined_score >= 70 else Fore.WHITE)
            )
...
def print_pipeline_result(
    ctx: PipelineContext, log_file: str | None = None, config: Config | None = None
) -> None:
    ...
    if n_pos > 0:
        print(
            Fore.GREEN + f"  PIPELINE COMPLETE — {n_pos} positions sized"
            f"  |  Strategy: {strategy}"
            f"  |  Run: {ctx.run_id}"
        )
    else:
        print(
            Fore.YELLOW + f"  PIPELINE COMPLETE — No actionable positions"
            f"  |  Strategy: {strategy}"
            f"  |  Run: {ctx.run_id}"
        )
