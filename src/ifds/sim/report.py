...
def print_validation_report(trades: list[Trade], summary: ValidationSummary) -> str:
    ...
    lines.append(
        f"  Execution Plans: {summary.plan_count}  |  Total Trades: {summary.total_trades}"
    )
    lines.append(_box_mid)

    # Fill rate
    fill_pct = summary.filled_trades / summary.total_trades * 100 if summary.total_trades > 0 else 0
    ...
    lines.append(
        f"  {W}LEG 1 (33% -> TP1/SL){RST}          Win Rate: {wr1_color}{summary.leg1_win_rate:.1f}%{RST}"
    )
    lines.append(
        f"    TP1 Hit: {summary.leg1_tp_hits}  |  Stop: {summary.leg1_stop_hits}  |  Expired: {summary.leg1_expired}"
    )
    lines.append(_box_mid)
    ...
    lines.append(
        f"  {W}LEG 2 (66% -> TP2/SL){RST}          Win Rate: {wr2_color}{summary.leg2_win_rate:.1f}%{RST}"
    )
    lines.append(
        f"    TP2 Hit: {summary.leg2_tp_hits}  |  Stop: {summary.leg2_stop_hits}  |  Expired: {summary.leg2_expired}"
    )
    lines.append(_box_mid)
    ...
    lines.append(
        f"    Total: {pnl_color}${summary.total_pnl:+,.2f}{RST}  |  "
        f"Avg/trade: ${summary.avg_pnl_per_trade:+,.2f}"
    )
    lines.append(
        f"    Best: {G}+${summary.best_trade_pnl:,.2f}{RST} ({summary.best_trade_ticker})"
...
def write_validation_trades(trades: list[Trade], output_dir: str = "output") -> str:
...
def write_validation_summary(summary: ValidationSummary, output_dir: str = "output") -> str:
    ...
    data = {
        ...
        "pnl": {
            ...
            "best": {"pnl": round(summary.best_trade_pnl, 2), "ticker": summary.best_trade_ticker},
            "worst": {
                "pnl": round(summary.worst_trade_pnl, 2),
...
def print_comparison_report(report: ComparisonReport) -> str:
    ...
    lines.append(
        f"    Trades: {bs.total_trades} | Filled: {bs.filled_trades}"
        f" | P&L: {pnl_c}${bs.total_pnl:+,.2f}{RST}"
    )
    lines.append(
        f"    Leg1 WR: {bs.leg1_win_rate:.1f}% | Leg2 WR: {bs.leg2_win_rate:.1f}%"
    ...
    for delta in report.deltas:
        ...
        lines.append(
            f"    Trades: {cs.total_trades} | Filled: {cs.filled_trades}"
        ...
        lines.append(
            f"    ΔP&L: {dc}${delta.pnl_delta:+,.2f}{RST}"
            f" | ΔAvg: ${delta.avg_pnl_delta:+,.2f}"
            f" | ΔFill: {delta.fill_rate_delta:+.1f}%"
        )
        lines.append(
            f"    ΔLeg1 WR: {delta.win_rate_leg1_delta:+.1f}%"
        ...
        if delta.insufficient_data:
            lines.append(
                f"    {Y}⚠ Insufficient data ({delta.paired_trade_count} paired trades < 30){RST}"
            )
            ...
            lines.append(
                f"    p-value: {sig_color}{delta.p_value:.4f} ({sig_label}){RST}"
...
def write_comparison_csv(report: ComparisonReport, output_dir: str = "output") -> str:
