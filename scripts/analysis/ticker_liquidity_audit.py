...
def categorize(stats: dict, total_days: int) -> str:
    ...
    # Category logic
    if dp_coverage >= 0.8 and opt_coverage >= 0.8 and persistence >= 0.5 and dp_pct_stability < 1.0:
...
def write_markdown(per_ticker: dict, total_days: int, snapshot_dates: list[str]) -> None:
    ...
    with open(OUTPUT_MD, "w", encoding="utf-8") as f:
        ...
        f.write(
            "is NOT captured because the Phase 4 snapshot does not currently persist these fields. "
        )
        f.write(
            "See `docs/tasks/2026-04-XX-phase4-snapshot-enrichment.md` for the follow-up task.\n\n"
        )
        ...
        f.write(
            "Tickers that consistently produce institutional data, "
            "sorted by number of days seen.\n\n"
        )
        a_sorted = sorted(cats["A"], key=lambda t: per_ticker[t]["days_seen"], reverse=True)[:50]
        f.write(
        ...
        f.write(
            "Tickers where the Phase 4 filter passed them at least once, "
            "but institutional data is essentially absent.\n\n"
        )
        d_sample = sorted(cats["D"], key=lambda t: per_ticker[t]["days_seen"], reverse=True)[:30]
        ...
        n_ab = n_a + len(cats["B"])
        f.write(
            f"The **Institutional Relevance Filter** for BC24 should consider "
        ...
        f.write(
            "Next step: extend snapshot enrichment to capture dp_volume ($), "
...
def main() -> None:
    ...
    print(
        f"  Loaded {total_days} snapshots ({dates[0]} → {dates[-1]}), "
