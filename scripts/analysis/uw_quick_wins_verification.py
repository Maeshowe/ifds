...
def _fetch(ticker: str, api_key: str, *, new_mode: bool) -> dict:
    ...
    try:
        ...
        if r.status_code != 200:
            return {"records": [], "status": r.status_code, "error": r.text[:200]}
        payload = r.json()
...
def _summarize(records: list[dict]) -> dict:
    ...
    if not records:
        return {"count": 0, "total_size": 0, "total_dollars": 0.0, "with_premium": 0}
    total_size = 0
    total_dollars = 0.0
    with_premium = 0
    for r in records:
...
def main() -> None:
    ...
    lines.append("")
    lines.append(
        "Compares OLD (limit=200, no UW-CLIENT-API-ID) vs "
    ...
    lines.append("")
    lines.append(
        f"- OLD: {totals_old['count']} records, "
        f"{totals_old['size']:,} shares, "
        f"${totals_old['dollars']:,.0f}"
    )
    lines.append(
        f"- NEW: {totals_new['count']} records, "
